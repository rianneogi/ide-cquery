...
class CppLanguageClient extends AutoLanguageClient {
  ...
  startServerProcess (projectPath) {
    ...
    const inputString = 'Content-Length: '.concat((JSON.stringify(initOptions).length).toString(),'\r\n\r\n')

    serverProcess.stdin.write(inputString)
    serverProcess.stdin.write(JSON.stringify(initOptions))
    // serverProcess.stdin.end()

    atom.notifications.addInfo(inputString, {dismissable: true, description:''})
    serverProcess.on('exit', exit => atom.notifications.addInfo('exited', {dismissable: true, description:'(exit.signal).toString()'}))
    serverProcess.on('message', message => atom.notifications.addInfo('hello', {dismissable: true, description:message.message}))
