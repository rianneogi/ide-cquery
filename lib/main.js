  ...
  preInitialization(connection) {
    ...
    if(doInactiveRegions)
    {
      connection.onCustom('$cquery/setInactiveRegions', (data) => {
        ...
        for(let i = 0;i<this.inactive_markers.length;i++)
        {
          this.inactive_markers[i].destroy()
        }
        this.inactive_markers.length = 0
        ...
        for(let i = 0;i<data.inactiveRegions.length;i++)
        {
          ...
          this.inactive_markers.push(marker)
        }
      })
    }
