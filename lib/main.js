...
class CqueryLanguageClient extends AutoLanguageClient {
  ...
  preInitialization(connection) {
    ...
    const doInactiveRegions = atom.config.get('ide-cquery.setInactiveRegions')
    ...
    const doSemanticHighlight = atom.config.get('ide-cquery.semanticHighlighting')
    if(doSemanticHighlight)
    {
      connection.onCustom('$cquery/publishSemanticHighlighting', (data) => {
        ...
        {
          if(data.symbols[i].kind==SymbolKind.Class || data.symbols[i].kind==SymbolKind.Struct)
          {
            options = {
              type: "text",
              class: "cquery--type"
          ...
          else if(data.symbols[i].kind==SymbolKind.Method || data.symbols[i].kind==SymbolKind.Constructor)
          {
            options = {
              type: "text",
          ...
          {
            let marker = editor.markBufferRange(Convert.lsRangeToAtomRange(data.symbols[i].ranges[j]))
            let decoration = editor.decorateMarker(marker, options)
          }
        }
      })
  ...
  postInitialization(server) {
    ...
    server.disposable.add(atom.workspace.onDidChangeActiveTextEditor(editor => {
      ...
      atom.workspace.observeTextEditors(editor => {
        this.customHighlights(editor)
        editor.onDidStopChanging(editor => {this.customHighlights(editor)})
      })
    }));
